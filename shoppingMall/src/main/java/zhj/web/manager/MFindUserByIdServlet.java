 ...
 * 管理端查询指定ID的用户
 */
@WebServlet("/mFindUserByIdServlet")
public class MFindUserByIdServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        String uid = request.getParameter("id");
        UserService service = new UserServiceImpl();
        User user = service.findById(uid);
        request.setAttribute("updateUser",user);
        request.getRequestDispatcher("/MupdateUser.jsp").forward(request,response);
    }
